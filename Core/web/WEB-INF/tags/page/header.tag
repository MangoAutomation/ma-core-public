...
<div id="mainHeader" data-dojo-type="dijit/layout/BorderContainer" style="width:100%; height: 85px" >
     ...
     <c:if test="${!simple}">
     <div id="alarmToaster" style="width:60%; height: 85px; border:0px; padding:.2em 0em 0em 5em;" data-dojo-type="dijit/layout/ContentPane" data-dojo-props="region:'center'">
     </div>
     </c:if>
     
     <div style="width:20%; border:0px; padding:0px;" data-dojo-type="dijit/layout/ContentPane" data-dojo-props="region:'trailing'">
        <c:if test="${!empty instanceDescription}">        
        <div style="position: relative; width: 100%; height: 100%">
 	    <span style="position: absolute; bottom:0px; right:0px; white-space:nowrap;" class="smallTitle marR">${instanceDescription}</span>
 	    </div>
        </c:if>
     </div>
<!-- Could put toolbar here later     <div data-dojo-type="dijit/layout/ContentPane" data-dojo-props="region:'bottom'">Bottom pane</div> -->
  ...
  require(["dojo/parser", "dijit/registry", "dojo/on", "dojo/topic", "dojo/dom-construct", 
           "dojo/dom", "dijit/layout/BorderContainer", "dijit/layout/ContentPane", "dojox/image", "dojo/domReady!"],
           function(parser,registry, on, topic, domConstruct, BorderContainer, ContentPane) {
	       ...
	       topic.subscribe("alarmTopic", function(message){
	           ...
	           if(message.type == 'clear')
	               ...
	               alarmMessageDiv.set('content', alarmMessageDiv.get('content') + message.message + "</br>");
	           }
	       });
	   });   
  </script>
