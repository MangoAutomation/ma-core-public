...
public class JsonDataDao extends AbstractVoDao<JsonDataVO, JsonDataTableDefinition> {
    ...
    public <R extends Record> SelectJoinStep<R> joinPermissions(SelectJoinStep<R> select, ConditionSortLimit conditions, PermissionHolder user) {
        if (!permissionService.hasAdminRole(user)) {
            ...
            Table<?> mintermsGranted = this.create.select(MintermsRoles.MINTERMSROLES.mintermId).from(MintermsRoles.MINTERMSROLES)
            ...
            Table<?> permissionsGranted = this.create.selectDistinct(PermissionsMinterms.PERMISSIONSMINTERMS.permissionId)
                    .from(PermissionsMinterms.PERMISSIONSMINTERMS).join(mintermsGranted)
                    .on(mintermsGranted.field(MintermsRoles.MINTERMSROLES.mintermId).eq(PermissionsMinterms.PERMISSIONSMINTERMS.mintermId))
                    .asTable("permissionsGranted");

            select = select.join(permissionsGranted).on(permissionsGranted.field(PermissionsMinterms.PERMISSIONSMINTERMS.permissionId)
