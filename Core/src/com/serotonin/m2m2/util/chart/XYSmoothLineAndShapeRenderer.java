...
public class XYSmoothLineAndShapeRenderer extends XYLineAndShapeRenderer {
	...
	protected void drawPrimaryLine(XYItemRendererState state, Graphics2D g2,
		...
		if (item == 1) {
			...
			point0.x = domainAxis.valueToJava2D(
					dataset.getXValue(series, item - 2), dataArea,
					xAxisLocation);
			point0.y = rangeAxis.valueToJava2D(
		...
		if ((item + 1) == dataset.getItemCount(series)) {
			...
			point3.x = domainAxis.valueToJava2D(
					dataset.getXValue(series, item + 1), dataArea,
					xAxisLocation);
			point3.y = rangeAxis.valueToJava2D(
					dataset.getYValue(series, item + 1), dataArea,
					yAxisLocation);
		}

		int steps = ((int) ((point2.x - point1.x) / 0.2) < 30) ? (int) ((point2.x - point1.x) / 0.2)
				: 30;
	...
	public static void getControlPoints(Point2D.Double point0,
		...
		Point2D.Double c1 = new Point2D.Double((point0.x + point1.x) / 2.0,
				(point0.y + point1.y) / 2.0);
		Point2D.Double c2 = new Point2D.Double((point1.x + point2.x) / 2.0,
				(point1.y + point2.y) / 2.0);
		Point2D.Double c3 = new Point2D.Double((point2.x + point3.x) / 2.0,
				(point2.y + point3.y) / 2.0);
		...
		Point2D.Double m1 = new Point2D.Double(c1.x + (c2.x - c1.x) * k1, c1.y
				+ (c2.y - c1.y) * k1);
		Point2D.Double m2 = new Point2D.Double(c2.x + (c3.x - c2.x) * k2, c2.y
				+ (c3.y - c2.y) * k2);

		control1.setLocation(new Point2D.Double(m1.x + (c2.x - m1.x) * smooth
				+ point1.x - m1.x, m1.y + (c2.y - m1.y) * smooth + point1.y
				- m1.y));
		control2.setLocation(new Point2D.Double(m2.x + (c2.x - m2.x) * smooth
				+ point2.x - m2.x, m2.y + (c2.y - m2.y) * smooth + point2.y
				- m2.y));
	}
	...
	public static Point2D.Double[] getBezierCurve(Point2D.Double point0,
		...
		Point2D.Double C = new Point2D.Double(3 * (control1.x - point1.x),
				3 * (control1.y - point1.y));
