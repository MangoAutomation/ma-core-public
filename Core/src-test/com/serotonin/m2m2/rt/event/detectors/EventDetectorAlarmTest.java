...
public class EventDetectorAlarmTest  extends MangoTestBase {
    ...
    static {
        ...
        Configurator.setLevel("com.serotonin.m2m2.rt.event.detectors.EventDetectorAlarmTest", Level.DEBUG);
        //Configurator.setLevel("com.serotonin.m2m2.rt.event.detectors.TimeoutDetectorRT", Level.DEBUG);
    }
        ...
        new Thread(() ->{
            runAs.runAs(runAs.systemSuperadmin(), () -> {
                ...
                for (int eventId = 0; eventId <= 202 ; eventId++) {
                    ...
                    newPvt = new PointValueTime(value ,timer.currentTimeMillis());
    ...
    private void testEventDetectorAssertAfter(int events  , int eventDetectorDuration, int eventDetectorDurationType) {
       ...
       for (int eventId = 1; eventId <= events ; eventId++) {
            timer.fastForwardTo(timer.currentTimeMillis() + 10000 + random.nextInt(5000));
    ...
    class EventDetectorEventListener implements UserEventListener {
        ...
        public void raised(EventInstance evt) {
            if(evt.getAlarmLevel() != AlarmLevels.INFORMATION){
                ...
                if (log.isTraceEnabled()) {
                    log.trace("Listener: Event " + evt.getId() + " is a " + evt.getAlarmLevel() + " time is: " + timer.currentTimeMillis());
                }
        ...
        public void returnToNormal(EventInstance evt) {
            ...
            if (log.isTraceEnabled())
                log.trace("Listener: Return to normal " + evt.getId() + " time is: " + timer.currentTimeMillis() );
